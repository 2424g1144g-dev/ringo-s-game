...
window.nonstopDebate1 = async function () {
  ...
  try {
    ...
    while (!signal.aborted) {
      ...
      await window.moveCameraPromise({to: {toX: 10, toY: 20, toZ: 0}, toFov: 40, fovSpeed: 0.2, speed: 100, yaw: -45, pitch: 0, rotSpeed: 0.2, duration: 3000}, signal);

      window.spawnFlexibleSerif("たしか...全員がおったよな？", 20, 60, window.letterByLetterPop, 3000);
      await window.moveCameraPromise({to: {toX: 10, toY: 20, toZ: 0}, toFov: 30, fovSpeed: 0.2, speed: 100, yaw: -45, pitch: 0, rotSpeed: 0.2, duration: 3000}, signal);
      loopCount++;
    }

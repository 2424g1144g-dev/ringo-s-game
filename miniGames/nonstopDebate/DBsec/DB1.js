...
window.nonstopDebate1 = async function () {
  ...
  try {
    ...
    while (!signal.aborted) {
      ...
      await window.moveCameraPromise({to: {toX: -10, toY: 16, toZ: 18}, toFov: 50, fovSpeed: 1, speed: 100, yaw: 130, rotSpeed: 5, duration: 500}, signal);

      window.ndIconChange("ササノ　ケント");
      window.spawnFlexibleSerif("しかも現場の近くには...", 55, 20, window.serifBehaviors.linearLeft, 3000);
      await window.moveCameraPromise({to: {toX: -10, toY: 16, toZ: 18}, toFov: 50, fovSpeed: 1, speed: 100, yaw: 140, rotSpeed: 5, duration: 3000}, signal); 

  ...
  cameraMove: async (event) => {
    ...
    if (event.slide) {
      if (event.slide === "X") {
        ...
        cameraMove({to: {toX: event.to.toX, toY: event.to.toY, toZ: event.to.toZ}, toFov: event.slideFov, fovSpeed: event.slideFovSpeed, yaw: event.slideYaw, rotSpeed: event.slideRotSpeed});
      }
    }
  },

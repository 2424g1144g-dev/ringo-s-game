...
function spriteChange(src, d = 100) {
  ...
  if (src === "none") return;
  setTimeout(() => {
    el.src = `characters/${src}.png`;
    console.log("srcにcharacters/",src,"pngを登録します" );
    setTimeout(() => {
      el.style.opacity = 1;
    }, d+100)
  }, d);
...
function createWave() {
    const original = document.getElementById('bullet');
    // 1. 元の弾丸をコピーする
    const wave = original.cloneNode(true); 
    // 2. IDが重複するとややこしいので消すか変更する
    wave.id = ""; 
    // 3. 波用のクラスをつけて、元の弾丸と同じ場所に配置
    wave.classList.add('wave');
    // 4. 画面に追加する
    original.parentNode.appendChild(wave);
    // 5. アニメーションが終わったら、分身を消してメモリを節約
